...
def modularity(partition, graph) :
    ...
    inc = dict([])
    deg = dict([])
    links = graph.size(weight = 'w')
    if links < 1 :
        return -2
    for node in graph :
        com = partition[node]
        deg[com] = deg.get(com, 0.) + graph.degree(node, weight = 'w')
...
def generate_dendogram(graph, part_init = None) :
    ...
    start = time.time()
    print "Starting to build the communities"
#    current_graph = graph.copy()
    ...
    print "one_level done : %d seconds elapsed" %(time.time() - start)
    new_mod = __modularity(status)
    partition = renumber(status.node2com)
    status_list.append(partition)
    mod = new_mod
    current_graph = induced_graph(partition, current_graph)
    print "Induced graph built : %d seconds elapsed" %(time.time() - start)
    ...
    while True :
        ...
        print "Pass nb%d : %d seconds elapsed" %(i, elapsed)
        one_level(current_graph, status)
        new_mod = __modularity(status)
        if new_mod - mod < MIN :
            break
...
def induced_graph(partition, graph) :
    ...
    """
    print "Building the induced graph"
    i = 0
    start = time.time()
    new_graph = nx.Graph()
    ...
    for node1, node2, datas in graph.edges_iter(data = True) :
        ...
        if i%10000000 == 0:
            elapsed = time.time() - start
            print "%d edges treated in %d seconds" %(i, elapsed)
    return new_graph
...
def one_level(graph, status) :
    ...
    while modif  and nb_pass_done != PASS_MAX :
        ...
        i = 0
        print "Starting pass nb %d : %d seconds elapsed" %(nb_pass_done, time.time() - start)
        for node in graph.nodes() :
            ...
            if i%100000 == 0:
            	elapsed = time.time() - start
            	print "%d nodes treated in %d seconds" %(i, time.time() - start)
        new_mod = __modularity(status)
...
class Status :
    ...
    def init(self, graph, part = None) :
        ...
        self.gdegrees = dict([])
        self.internals = dict([])
        self.total_weight = graph.size(weight = 'w')
        if part == None :
            for node in graph.nodes() :
                self.node2com[node] = count
                deg = float(graph.degree(node, weight = 'w'))
